        ...
        ajax.requestLoad(url, data, 'GET').then(res => {
            if (res.code === 20000) {
                ...
                for (var i in question_data.options) {
                    question_data.options[i] = '<div style="white-space:pre-wrap">' + String.fromCodePoint(parseInt(i) + 65) + '.&nbsp;' + changeStr.changeReplace(question_data.options[i]) + '</div>'
                }
                //如果有小题
                if (question_data.children.length > 0) {
                    ...
                    question_data.children.map((itch, idch) => {
                        ...
                        // console.log(itch.indexChildstem)
                        //给每一个答案，拼接 A B C D
            ...
            pageData.children.map((itch, ic) => {
                if (itch.audio != null && itch.audio != "" && itch.qid == qid) {
                    ...
                    innerAudioContext.onEnded(() => {
                        that.setData({
                            [`pageData.children[${ic}].currentTime`]: '00:00',
